...
# 策略核心：中大市值股 + 放量启动 + 早盘竞价爆量重排（防杂毛优化版）
# 针对痛点优化：
# 1. 痛点：早盘竞价重排极易选到“无量空拉、买卖比作假、成交仅几百万”的僵尸杂毛股，开盘即高开低走被套。
# 2. 竞价量比强制限制：高开超过 1.0% 时，竞价成交量必须达到昨日成交量的 1.0% 以上，否则重罚 25 分，彻底拦截虚假诱多。
# 3. 竞价金额强力过滤：绝对竞价成交额不足 300 万扣 10 分，不足 500 万不给任何高开加分。只有真金白银交易的股票才能拿高分。
# 4. 挂单比例防作弊：买一/卖一挂单比只在成交额达 500 万以上时生效，防小盘股薄盘虚假挂单对敲。
# 5. 中大市值放量启动：延续 80亿-300亿 流通市值定位，寻找主流放量强势上升的股票。
# 6. 避险与持仓优化：1月/4月年报排雷全仓空仓避险；昨日涨停个股今日尾盘封板则不卖。
...
def before_trading_start(context):
    ...
    log.info(f"--- 正在执行 {yesterday} 的中大市值盘前复盘选股 ---")
    ...
    # 获取全市场候选股的成交额，筛选前 100 只最活跃的热门股作为核心监控池
    ...
    # 取成交额排名前 100 的股票作为核心监控池 (monitor_pool)
    ...
    for code in candidate_pool:
        ...
        # B. 市值打分 (偏好80亿~300亿市值，惩罚偏小或偏大) -> Max 10分
...
def calc_auction_score(yesterday_close, auction, price_info, yesterday_volume=0.0):
    ...
    2. 量能爆发加分：竞价量比 >= 2.0%（+5分），>= 3.5%（+10分，主力强抢筹）。
