# 克隆与修改自：首板与断板反包竞价融合策略.py
# 融合策略名称：首板与断板反包竞价融合策略_自选重排版
# ========================================================================
# 核心思路：
...
#      - Setup 3 (三日断板)：大前日首板，前日昨日断板，昨日回调小于5%，匹配 CONDITION_RULES_SETUP2。
#   3. 历史自选重排支持：也把自选排行榜中的历史候选个股加入每日开盘竞价，进行重排评分过滤。
#   4. 风控体系：完全整合原版ML风控与净值曲线动量控制。
# ========================================================================
...
# -------------------- 6. 历史自选参与竞价配置 --------------------
ADD_TRACKED_TO_AUCTION = True      # 是否将历史自选股加入每日竞价重排池
TRACKED_LOOKBACK_DAYS = 15          # 历史自选参与竞价的追溯天数限制 (例如仅最近15个交易日内入选过的自选股参与，防止股票池无限膨胀；若设置为 999 代表包含所有历史候选股)
...
def get_buy(context):
    ...
    # 1. 过滤并加载参与竞价的目标股票池（融合今日形态初筛 + 历史自选追踪股）
    tracked_list = []
    if ADD_TRACKED_TO_AUCTION:
        # 按照 entry_date 过滤最近 TRACKED_LOOKBACK_DAYS 天内的自选股，避免池子无限扩张
        recent_trade_days = get_trade_days(end_date=context.previous_date, count=TRACKED_LOOKBACK_DAYS)
        recent_trade_days_str = {d.strftime('%Y-%m-%d') for d in recent_trade_days}
        
        for code, cand in g.tracked_candidates.items():
            if cand['entry_date'] in recent_trade_days_str:
                tracked_list.append(code)
                
    # 取并集并去重
    all_targets = list(set(g.target_setup1 + g.target_setup2 + g.target_setup3 + tracked_list))
    if not all_targets:
    ...
    log.info(f"【竞价开始】共有 {len(g.target_setup1)}只 Setup 1 候选，{len(g.target_setup2)}只 Setup 2 候选，{len(g.target_setup3)}只 Setup 3 候选，以及 {len(tracked_list)}只自选追踪加入竞价重排")
    ...
    for s in all_targets:
        ...
        # 确定适用的规则类型和标签
        if s in g.target_setup1:
            stype = '1进2'
            rules = CONDITION_RULES_SETUP1
        elif s in g.target_setup2:
            stype = '断板反包'
            rules = CONDITION_RULES_SETUP2
        elif s in g.target_setup3:
            stype = '三日断板'
            rules = CONDITION_RULES_SETUP2
        else:
            orig_type = g.tracked_candidates[s].get('setup_type', '断板反包')
            stype = f"自选({orig_type})"
            rules = CONDITION_RULES_SETUP2  # 历史自选股默认使用更宽容的 Setup 2 规则
            
        try:
            prev = prev_map.get(s)
            if prev is None:
                log.info(f"  [排除-{stype}] {s}({name}) 未能获取昨日日线数据")
                continue
                
            # 针对 1进2 候选执行额外的首板均价硬度过滤
            if stype == '1进2':
                avg_chg = prev['money'] / prev['volume'] / prev['close'] * 1.1 - 1
                if avg_chg < 0.035:
                    log.info(f"  [排除-1进2] {s}({name}) 昨日均价涨幅 {avg_chg:.2%} < 3.5%")
                    continue
            ...
            if open_price <= 3:
                log.info(f"  [排除-{stype}] {s}({name}) 开盘价 {open_price} <= 3")
                continue
            if val is None:
                log.info(f"  [排除-{stype}] {s}({name}) 未能获取市值数据")
                continue
            if val['market_cap'] < MIN_CAP or val['circulating_market_cap'] > MAX_CAP:
                log.info(f"  [排除-{stype}] {s}({name}) 市值不符: 总市值={val['market_cap']:.1f}亿, 流通={val['circulating_market_cap']:.1f}亿")
                continue
            if money < MIN_AMOUNT or money > MAX_AMOUNT:
                log.info(f"  [排除-{stype}] {s}({name}) 昨日成交额 {money/1e8:.2f}亿 不在 [{MIN_AMOUNT/1e8:.1f}亿, {MAX_AMOUNT/1e8:.1f}亿] 区间")
                continue
        except Exception as e:
            log.info(f"  [排除-{stype}] {s}({name}) 基础过滤异常: {e}")
            continue
        ...
        try:
            ...
            if len(vol_data) < 2:
                log.info(f"  [排除-{stype}] {s}({name}) 历史成交量天数不足")
                continue
        except Exception as e:
            log.info(f"  [排除-{stype}] {s}({name}) 成交量过滤异常: {e}")
            continue
        ...
        try:
            ...
            if auction.empty:
                log.info(f"  [排除-{stype}] {s}({name}) 获取竞价数据为空")
                continue
            ...
            # OBI 力量比过滤
            if obi_ratio < 0.6:
                log.info(f"  [排除-{stype}] {s}({name}) 竞价买卖比不符: 买方资金={buymoney/1e4:.1f}万, 卖方资金={sellmoney/1e4:.1f}万, 比例={obi_ratio:.2f} < 0.6")
                continue
            ...
            if matched_condition is None:
                log.info(f"  [排除-{stype}] {s}({name}) 竞价未匹配成功: 竞价涨幅={(cur_ratio-1)*100:.2f}%, 竞昨比={auction_ratio*100:.2f}% (成交额={money/1e8:.2f}亿)")
                continue
            ...
            qualified_stocks.append({
                ...
                'name': name,
                'score': score,
                'type': f"{stype}({matched_condition})"
            })
            log.info(f"✅ {s}({name}) 符合 {stype}，命中条件: {matched_condition} | 得分: {score:.2f} | 换手: {turnover_ratio:.2f}% | OBI: {obi_ratio:.2f}")
        except Exception as e:
            log.info(f"  [排除-{stype}] {s}({name}) 竞价匹配异常: {e}")
            continue
