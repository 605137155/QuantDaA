...
# 策略核心：中等偏小市值黄金区间（30亿-100亿） + 放量启动 + 竞价爆量重排（防杂毛优化版）
# 黄金市值平衡点定位：
# 1. 痛点：小微盘（<30亿）竞价容易买入“没有成交量、容易被操纵、流动性差”的杂毛股；中大盘（>100亿）个股盘子太重，缺乏超短弹性。
# 2. 市值黄金区间：严格筛选流通市值在【30 亿到 100 亿】之间的股票。该区间是游资与主力最喜爱的弹性温床，流动性充足（防杂毛），且足够轻盈（具爆发力）。
# 3. 竞价量比强制限制：高开超过 1.0% 时，竞价成交量必须达到昨日成交量的 1.0% 以上，否则重罚 25 分，彻底拦截虚假诱多。
# 4. 竞价金额强力过滤：绝对竞价成交额不足 300 万扣 10 分，不足 500 万不给任何高开加分。只有真金白银交易的股票才能拿高分。
# 5. 避险与持仓优化：1月/4月年报排雷全仓空仓避险；昨日涨停个股今日尾盘封板则不卖。
...
def initialize(context):
    ...
    # 2. 每天早上 9:33 执行早盘竞价重排买入（9:33可确保集合竞价接口数据完整生成，防止获取失败）
    run_daily(my_morning_trade, time='09:33')
    # 3. 每天下午 14:50 执行尾盘清仓昨日持仓
    run_daily(my_afternoon_trade, time='14:50')

# ==================== 2. 每日交易前准备（选股打分） ====================
def before_trading_start(context):
    # 如果检测到今天属于空仓避险时段，直接跳过选股
    if g.no_trading_today_signal:
        g.scored_candidates_all = []
        log.info("【空仓避险】今日处于1月或4月高风险期，跳过选股。")
        return

    yesterday = context.previous_date.strftime('%Y-%m-%d')
    log.info(f"--- 正在执行 {yesterday} 的中等市值黄金区间选股打分 ---")
    ...
    # 2. 批量获取全市场的流通市值，并严格筛选出【30亿到100亿】流通市值的黄金个股
    q = query(
        valuation.code,
    ...
    df_cap = df_cap[(df_cap['circulating_market_cap'] >= 30.0) & (df_cap['circulating_market_cap'] <= 100.0)]
    sweet_spot_stocks = df_cap['code'].tolist()
    cap_dict = dict(zip(df_cap['code'], df_cap['circulating_market_cap']))
    
    # 3. 从符合市值要求的股票中，筛选前 100 只最活跃的热门股作为最终候选池（ monitor_pool ）
    h_amount_all = history(1, '1d', 'money', sweet_spot_stocks, df=False)
    amount_all_dict = {code: float(h_amount_all[code][0]) for code in sweet_spot_stocks if len(h_amount_all[code]) > 0}
    ...
    log.info(f"符合30亿-100亿流通市值且昨日成交额排名前100的候选股数: {len(candidate_pool)}")
    ...
    # 4. 批量获取候选热门股的65天日线数据（缩短为65天，原超跌策略需要105天数据）
    ...
    for code in candidate_pool:
        ...
        # B. 市值打分 (偏好弹性最高的 30亿 - 70亿 区间) -> Max 10分
...
def my_morning_trade(context):
    """早盘9:33交易逻辑 (结合集合竞价表现重排打分，开盘立即买入，排除假高开杂毛)"""
    if g.no_trading_today_signal:
        return
    ...
    for item in g.scored_candidates_all:
        ...
        # 创新防杂毛竞价打分器
        auction_score, auction_pct, auction_money, vol_ratio = calc_auction_score(
            yesterday_close, 
            auction, 
            price_info, 
            yesterday_volume
        ...
        # 复合得分 = 盘前得分 + auction_score - 0.5 * 昨日振幅
    ...
    # 选取复合得分前 g.max_stocks 的个股作为买入目标
    morning_target_list = [item['code'] for item in sorted_candidates][:g.max_stocks]
    
    # 打印选定买入个股的详细信息
    log.info(f"今日早盘 9:33 竞价重排选股结果：")
    for i, code in enumerate(morning_target_list):
        item = next(x for x in sorted_candidates if x['code'] == code)
        log.info(f"选定买入第 {i+1} 顺位: {code} ({item['name']}) - 复合分: {item['composite_score']:.2f} | 原始分: {item['original_score']} | 竞价分: {item['auction_score']:.2f} | 竞价高开: {item['gap_pct']:.2%}| 竞价量比: {item['vol_ratio']:.2%}| 竞价金额: {item['auction_money']/10000.0:.1f}万 | 昨日振幅: {item['amplitude']:.2f}%")
        
    # 获取可用资金（昨日下午清仓后释放的资金，保留1%缓冲）
...
def my_afternoon_trade(context):
    ...
    for stock in current_holdings:
        ...
        # 昨日涨停且今日尾盘依然牢牢封死涨停，选择保留
        if stock in yesterday_HL_list:
            if current_data[stock].last_price >= current_data[stock].high_limit - 0.01:
                log.info(f"【继续持有】昨日涨停股 [{stock}] 今日尾盘封板未开，继续持有")
                continue
            else:
                log.info(f"【涨停打开卖出】昨日涨停股 [{stock}] 今日尾盘封板打开，清仓卖出")
...
def calc_market_cap_score(circ_cap):
    """黄金市值打分逻辑 (首选 30亿 - 70亿 弹性最大区间，限制在 30亿 - 100亿 黄金底色)"""
    if circ_cap <= 0:
        return 0
    if 30.0 <= circ_cap <= 70.0:
        return 10  # 黄金小市值加成，弹性最高且无流动性风险
    elif 70.0 < circ_cap <= 90.0:
        return 7
    elif 90.0 < circ_cap <= 100.0:
        return 4
    return 0
...
def get_today_auction_map(codes, today):
    ...
    try:
        ...
        log.info(f"【竞价数据】get_call_auction 不可用，将使用开盘行情兜底：{exc}")
    return result
...
def calc_auction_score(yesterday_close, auction, price_info, yesterday_volume=0.0):
    """
    超强集合竞价防杂毛打分器：
    1. 提高量比门槛：高开 >= 1.0% 时，竞价量比必须 >= 1.0%（原为0.3%），否则视为虚假诱多，直接扣 25 分。
    2. 量能爆发加分：竞价量比 >= 2.0%（+5分），>= 3.5%（+10分，主力强行抢筹）。
    3. 提升成交额绝对门槛：绝对成交额不足 300 万扣 10 分，不足 500 万不给任何高开加分，防止小成交量杂毛。
    4. 成交额阶梯大资金加分：>= 1500万（+6分），>= 3000万（+10分）。
    5. 超高开无量重罚：高开 >= 7.0% 且量比 < 2.0%，扣 25 分，规避高开低走接盘。
    6. 买卖挂单比降权防操纵：买一/卖一挂单比只在成交额 >= 500 万时生效，且最高降为 4 分，防止庄家虚假堆单。
    """
    ...
    if auction_volume <= 0 and auction_price > 0:
        auction_volume = auction_money / auction_price
    
    vol_ratio = (auction_volume / yesterday_volume) * 100.0 if yesterday_volume > 0 else 0.0

    score = 0.0

    # 3. 基础涨幅加分（加入成交额门槛：竞价额必须 >= 500万，否则视为无主力杂毛高开，不予加分）
    if auction_money >= 5000000.0:
        if 1.0 <= auction_pct <= 5.5:
            score += 15.0  # 黄金高开区间
        elif (0.0 <= auction_pct < 1.0) or (5.5 < auction_pct <= 7.0):
            score += 8.0   # 温和高开
    elif auction_pct < 0.0:
        score -= 15.0  # 低开惩罚（不论金额）

    # 4. 强力过滤无量假高开（降妖除魔核心逻辑）
    if auction_pct >= 1.0:
        if vol_ratio < 1.0: # 高开但竞价成交量占昨日总成交量低于 1%
            score -= 25.0  # 重罚，排除无量诱多
            
    # 5. 强力过滤超高开无量抢筹（防接盘）
    if auction_pct >= 7.0 and vol_ratio < 2.0:
        score -= 25.0      # 重罚，排除高位诱多
    ...
    # 7. 竞价绝对金额打分 (提高大资金主力参与感)
    if auction_money >= 30000000.0:     # >= 3000万
        score += 10.0
    elif auction_money >= 15000000.0:   # >= 1500万
        score += 6.0
    elif auction_money >= 5000000.0:    # >= 500万
        score += 2.0
    elif auction_money < 3000000.0:     # < 300万
        score -= 10.0                   # 绝对成交额过低，扣分排除僵尸杂毛

    # 8. 买一卖一挂单比例打分 (必须在大资金成交支撑下才计算，防主力做假账)
    if auction_money >= 5000000.0:
    ...
    return score, auction_pct, auction_money, vol_ratio / 100.0
