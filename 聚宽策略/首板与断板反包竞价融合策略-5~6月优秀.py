...
# 优化：放宽竞昨比上下限，打通原版不连续的量能“盲区/死区”
CONDITION_RULES_SETUP1 = [
    ('A: 竞价高开7~9% | 竞昨比2.5~25%',  1.07, 1.09, 0.025, 0.25),
    ('B: 竞价高开4~7% | 竞昨比2~25%',   1.04, 1.07, 0.02, 0.25),
    ('C: 竞价平开至小高开0~4% | 竞昨比1.5~15%', 1.00, 1.04, 0.015, 0.15),
]

# 2.2 Setup 2 (断板反包) 的竞价规则
# 优化：降低竞昨比下限至 1.0% 避免误杀低量能反包股，拓宽上限以包容早盘高人气股
CONDITION_RULES_SETUP2 = [
    ('反包A: 竞价高开4~8% | 竞昨比1~20%', 1.04, 1.08, 0.01, 0.20),
    ('反包B: 竞价高开2~4% | 竞昨比1~15%', 1.02, 1.04, 0.01, 0.15),
    ('反包C: 竞价平开至小高开0~2% | 竞昨比1~12%', 1.00, 1.02, 0.01, 0.12),
    ('反包D: 竞价低开-3~0% | 竞昨比1~12%', 0.97, 1.00, 0.01, 0.12),
]
...
def initialize(context):
    ...
    g.target_setup2 = []           # Setup 2: 断板反包候选
...
def before_market_open(context): 
    ...
    # 获取过去8个交易日的数据（以判断前6个交易日的首板状态）
    trade_days = get_trade_days(end_date=context.previous_date, count=8)
    if len(trade_days) < 8:
        log.info("[选股] 交易历史不足8天，跳过选股")
        g.target_setup1 = []
        g.target_setup2 = []
        return
        
    t_minus_8 = trade_days[0]
    t_minus_1 = trade_days[-1]
    
    price_df = get_price(
        initial_list, start_date=t_minus_8, end_date=t_minus_1, frequency='1d',
    ...
    d_1 = trade_days[7].strftime('%Y-%m-%d') # T-1
    d_2 = trade_days[6].strftime('%Y-%m-%d') # T-2
    d_3 = trade_days[5].strftime('%Y-%m-%d') # T-3
    recent_6_days = [d.strftime('%Y-%m-%d') for d in trade_days[2:8]] # T-6 到 T-1
    ...
    log.info(f"[选股] 形态初筛完成. Setup 1 (1进2): {len(raw_setup1)}只 | Setup 2 (断板反包): {len(raw_setup2)}只")
    ...
    # 构建名字缓存与输出日志
    g.name_cache = {}
    all_targets = g.target_setup1 + g.target_setup2
...
def get_buy(context):
    ...
    log.info(f"【竞价开始】共有 {len(g.target_setup1)}只 Setup 1 候选，{len(g.target_setup2)}只 Setup 2 候选")
...
def compute_ml_features(context):
    ...
    # 候选股票热度
    f4 = float(len(g.target_setup1) + len(g.target_setup2))
